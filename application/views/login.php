...
<?=$this->session->flashdata('pesan_error');?>
	...
	<div class="inputGroup inputGroup2">
		<label for="loginEmail" id="loginEmailLabel">NO HP</label>
		<input type="text" id="loginEmail" maxlength="15" name="no_hp" autofocus/>
...
<script type="text/javascript">
      ...
      function isEmail(no_hp) {
        var regex = /[0-9]/;
        return regex.test(no_hp);
      }

		var no_hp = $('#loginEmail').val().trim();
		var password = $('#loginPassword').val().trim();
    

		if(no_hp == ''){
			$('#pesan_error').html('<div class="alert alert-danger text-center">Isi <strong>No HP</strong> Kamu</div>');
			$('#loginEmail').focus();
		} else if(!isEmail(no_hp)){
      $('#pesan_error').html('<div class="alert alert-danger text-center">Masukkan Hanya <strong>Angka (123456)</strong></div>');
      $('#loginEmail').focus();
    } else if(no_hp.length < 10 ) {
			$('#pesan_error').html('<div class="alert alert-danger text-center"><strong>No HP</strong> harus 10 karakter atau lebih</div>');
			$('#loginEmail').focus();
		} else if(password == '') {
			$('#pesan_error').html('<div class="alert alert-danger text-center">Isi <strong>Password</strong></div>');
			$('#loginPassword').focus();
  ...
  <script src='<?=base_url('template/assets/js/TweenMax.min.js');?>'></script>
