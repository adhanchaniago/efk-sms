...
function search() {
 ...
 $.ajax({
  ...
  url: "http://172.16.1.187/er-tracking/WebService/getWSBR", // Isi dengan url/path file php yang dituju
  data: {
  ...
  $("#htxt_noer").val(response.txt_noer);
    ...
    $("#txt_checker").val(response.txt_checker).trigger("chosen:updated");
    ...
    $("#txt_tipeerpr").val(response.txt_tipeerpr).trigger("chosen:updated");
    //$("#txt_dep").val(response.txt_dep);
     $("#txt_dep").val(response.txt_dep).trigger("chosen:updated");
...
$("#branch").val(branch);
   ...
   document.getElementById("slapr").innerHTML = convertdate(response.txt_appdate, response.txt_prdate);
    ...
    document.getElementById("txt_inv").disabled = true;
